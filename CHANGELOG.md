...
- Java callgraph contracts now model the Spring Security Crypto 7.1 encoder surface as a distinct version-pinned KB (`spring-security-crypto-7.1.yaml`), additive to the existing 6.3.4 contract: the new `org.springframework.security.crypto.password4j.*` Password4j-backed `PasswordEncoder` wrappers (factory constructors only, encode/matches resolved purely via the pre-existing `PasswordEncoder` hierarchy), the `PasswordEncoderFactories.createDelegatingPasswordEncoder` default-encoder factory, the hand-rolled `org.springframework.security.crypto.bcrypt.BCrypt` static hashpw/checkpw/gensalt terminal, and the previously-uncontracted `AesBytesEncryptor` constructors and `Encryptors.delux` text-encryptor factory. (#182)
- Java callgraph contracts now model Nimbus JOSE+JWT 10.0.2's JSON serialization lifecycle (`JOSEObjectJSON`/`JWSObjectJSON`/`JWEObjectJSON` and their nested `Signature`/`Recipient` value types), additive to the version-pinned 9.37.3 contract: parse/construct, sign/verify, encrypt/decrypt, and general/flattened serialization roles. (#184)
- Java callgraph contracts now model Apache Commons Codec's hand-rolled BLAKE3 lifecycle: the three mode-selecting factories (initHash/initKeyedHash/initKeyDerivationFunction), the one-shot hash/keyedHash statics, and the self-returning update/doFinalize/reset fluent chain, joining the crypto_rules#164 hash/mac/kdf rule anchors. (#187)
- Java callgraph contracts now model jose4j's JWE key-management algorithm factories (RSA-OAEP/RSA1_5, AES key wrap, AES-GCM key wrap, PBES2, direct), the shared KeyManagementAlgorithm/ContentEncryptionAlgorithm operation interfaces, the hand-rolled AES-CBC-HMAC content-encryption composite, the hand-rolled ConcatKDF and PBKDF2 key-derivation classes, and the RSA/EC/oct/OKP JWK generators; the P1 verification/consumption surface (JwtConsumerBuilder, JsonWebSignature, JsonWebEncryption, JWKS resolvers) is deferred pending crypto_rules#174. (#188)
- Java callgraph contracts now model Bouncy Castle bcpkix 1.84 builder lifecycles, declared separately from the bcprov lightweight coverage: X.509 certificate issuance (X509v3CertificateBuilder and its Jca/Bc convenience subclasses), the JcaContentSignerBuilder/BcContentSignerBuilder and JcaContentVerifierProviderBuilder/BcContentVerifierProviderBuilder operator builders, CMSSignedDataGenerator/CMSEnvelopedDataGenerator and the SignerInfoGenerator/content-encryptor builders that feed them, the PKCS#10 certification-request builder, and the OCSPReqBuilder request lifecycle. (#185)
