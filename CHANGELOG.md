...
- A callgraph contract KB for the `aws-sdk-kms` crate (the generated AWS SDK for Rust's KMS client), so a consumer's encrypt, decrypt, re-encrypt, sign, verify, MAC, data-key, shared-secret and random call sites resolve to a declared signature instead of an empty one. The cryptography is the service's, not the caller's, so nothing here claims a local runtime provider. Every KMS call is a fluent chain, so 147 builder links are declared with `role: config` alongside the operations -- each plain setter, its documented `set_*` override form and each builder's `customize()` -- because an undeclared link collapses everything after it: the operation resolves and every later hop falls back to the SCANNED crate's own name, which is a wrong identity rather than a missing one. Measured on a realistic consumer: 7 of 21 call sites carried an aws-sdk-kms identity before, 21 of 21 after. The range starts at 0.25.0, the release in which the generated builders take the `operation::<op>::builders::<Op>FluentBuilder` spelling this file declares; the six `*encryption_context` setters are arity 2, since they take a key and a value.
