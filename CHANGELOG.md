...
### Fixed
- Python parser throughput: the single-file walk now visits each syntax node once, instead of the prior multi-pass traversal, measurably reducing parse time and allocations on large Python codebases. (openspec/changes/python-parser-parity-2)
- Python callgraph reachability and supporting-call precision now matches Java's structural coverage:
  ...
  - `self.attr`/`cls.attr` assigned in any method of a class now resolves as a stable receiver identity shared across that class's methods, instead of being invisible to reachability.
  - Module-level and class-body statements that call crypto APIs outside any function/method now get a synthetic `<module>`/`<clinit>` entry point, so those calls are reachable instead of silently dropped. This synthetic entry point's receiver table is scoped to that module/class body's own direct statements only — a variable bound only inside a nested function or method is never mistaken for one of its receivers.
  - Import resolution now recurses into `try`/`except`, `if` (e.g. `TYPE_CHECKING` guards), and function bodies instead of only the file's top-level statements, and correctly resolves relative imports (`from . import x`, `from ..pkg import y`) against the current package instead of mis-binding or dropping them. A plain dotted import (`import a.b.c`) now binds only the top-level module name, matching real Python import-binding semantics, instead of binding the full dotted path and both mis-resolving a later chained-attribute call and silently hiding a subsequent distinct `import a.d`.
  - A package's `__init__.py` re-exports (`from .mod import Name`) are now propagated so a sibling module's `from pkg import Name` resolves to the symbol's real declaring module, gated on in-graph evidence and restricted to project-local packages so it never rewrites a versioned dependency's public knowledge-base-keyed re-export path.
  - An UPPER_CASE local variable (e.g. a constant-style instance name) now resolves as a call receiver instead of being mistaken for a class/type reference.
  - No export schema change; `internal/scan/supporting_calls.go` is unchanged. (openspec/changes/python-parser-java-parity)
