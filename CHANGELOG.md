...
## [Unreleased]
### Added
- A callgraph contract KB for the `bls12_381` crate (the zkcrypto implementation of the BLS12-381 pairing-friendly curve), so a consumer's pairing, scalar-sampling, hash-to-curve and point-encoding call sites resolve to a declared signature instead of an empty one. All 23 entries declare `parameter_types` and `canonical_return_type`, and they cover both routes into the pairing: the crate-root free functions `pairing` and `multi_miller_loop`, and the `pairing::Engine`/`MultiMillerLoop` implementations on `Bls12`. The declared range is `>=0.5.0,<0.9.0`, narrower than the crate's published history because `version_range` is declared per library and may claim only versions for which every entry holds: `hash_to_curve` arrives at 0.5.0, and before 0.3.0 the four `random` entries take `&mut R` rather than the declared `impl RngCore`. Note that `hash_to_curve` sits behind the crate's non-default `experimental` feature, so those four entries describe a surface a consumer opts in to compile; the other nineteen are on by default.
- A callgraph contract KB for the RustCrypto `chacha20` crate (ChaCha8/12/20, XChaCha, the legacy 64-bit-nonce djb variant, and the crate's own rng-feature adapters), so a consumer's stream-cipher construction, keystream, seek and RNG call sites resolve to a declared signature instead of an empty one. 81 entries span `>=0.1.0,<0.11.0`: the type names are stable across that range, so constructor spellings follow the trait crate each era re-exports (`new` everywhere, `new_var` through 0.6.0, `new_from_slices` from 0.7.0, and `XChaCha8`/`XChaCha12` postdate `new_var`, so those two combinations are deliberately absent). Where method plus arity does not identify one declared signature across the range, the era-varying field is omitted rather than declared for one era: `apply_keystream_b2b` returns `Result` under `cipher` 0.4 and is infallible under `cipher` 0.5, `SeedableRng::from_rng` returns `Result` under rand_core 0.5/0.6 and `Self` from 0.9, `seek`/`try_seek` take a `SeekNum` that real consumers spell as both `u32` and `u64`, and `hchacha`'s 16-byte input moved from `GenericArray` to hybrid-array `Array`. `rand_chacha` publishes the same `ChaCha20Rng` type names from a different crate and never resolves here, because the callee identity carries the consumer's crate path.
- `Self::Variant(binding)` match arms now bind the variant's payload type instead of the enclosing enum. `rustVariantKey` built the facts key from the written path (`Self::Cha`) while a variant's fields are recorded under the declared name (`Stream::Cha`), so the lookup failed and the binding fell back to the enum type itself. A dispatch wrapper that matches on `self` — the shape a KeePass database reader publishes around its salsa20 and chacha20 streams — therefore had every arm's receiver typed as the consumer's own enum, and the foreign-receiver filter dropped the genuine dependency calls made through it. The `Enum::Variant(binding)` spelling is unaffected and is pinned alongside the fix.
- A callgraph contract KB for the `boringtun` crate (Cloudflare's userspace WireGuard), so a consumer's tunnel construction and data-path call sites resolve to a declared signature instead of an empty one. `Tunn` is never re-exported from the crate root, so every key keeps its `boringtun::noise` module segment. Both data-path eras are declared as distinct keys, because 0.3.0 renamed `tunnel_to_network`/`network_to_tunnel` to `encapsulate`/`decapsulate` and no version ships both; `Tunn::new` is declared at both of its arities (five at 0.2.0, six from 0.3.0). The arity-6 constructor declares no `canonical_return_type` and no `parameter_types`, and both omissions are the honest option rather than a gap: its declared return changed at 0.7.0 (`Result` to `Self`) and its key parameter types changed at 0.5.0 (the crate's own `X25519SecretKey` to `x25519_dalek::StaticSecret`) without a name or arity change, and a KB entry holds one value per method+arity. `return.type` still declares `boringtun::noise::Tunn`, which is what a chain walk needs. The range starts at 0.2.0: release 0.0.0 is a name placeholder with no library at all.
- A callgraph contract KB for the `rcgen` crate (X.509 certificate, CSR and CRL generation), so a consumer's certificate issuance and signing-key call sites resolve to a declared signature instead of an empty one. Every type is re-exported from the crate root, so the keys carry no module segment; the free function `generate_simple_self_signed` is authored in the dot form because a single-dot key is looked up raw rather than rewritten. Six methods are declared at two arities each, because rcgen moved its signing verb twice: `KeyPair::generate` takes the signature algorithm through 0.12.1 and none from 0.13.0; `signed_by` loses an argument at 0.14.0 on `CertificateParams`, `CertificateSigningRequestParams` and `CertificateRevocationListParams` alike, when an explicit `Issuer` absorbs the issuing key; and `CertificateParams::from_ca_cert_pem` / `_der` drop their key-pair argument at 0.13.0. `CertifiedIssuer`, which arrives at 0.14.3, is contracted at both of its associated functions. The encoding methods are declared with `role: output` and are matched by no rule, because `serialize_der` signs on a `Certificate` receiver in 0.1.0-0.12.1 and only encodes on a `KeyPair` one — a distinction a rule cannot make and a contract does not need to, since it is reached only through a receiver already typed as this crate's. The range starts at 0.1.0: release 0.0.2 publishes no `src/lib.rs` at all.
- A callgraph contract KB for the `blstrs` crate (BLS12-381 over the blst assembly backend), so a consumer's pairing, Miller-loop and point-codec call sites resolve to a declared signature instead of an empty one. All 34 entries declare a return type and, where the call takes arguments, `parameter_types`. The declared range is `>=0.4.0,<0.8.0` rather than the crate's whole published history, and the bound is the honest one rather than a convenient one: blstrs changes the RETURN of `pairing` (`Fp12` to `Gt`) and of every point decoder (`Option` to `subtle::CtOption`) at 0.4.0 without changing name or arity, a KB entry is keyed by method plus arity, and declaring one era's return over the whole range would publish something false for half of it. 0.1.0-0.3.1 is therefore uncontracted and says so in the file. Declaring `multi_miller_loop`'s return is what types a chained `.final_exponentiation()` on `MillerLoopResult` rather than on `Bls12`, which is where the parser put it while the return was unknown.
- A callgraph contract KB for the RustCrypto `hkdf` crate (RFC 5869 extract-and-expand), so a consumer's key-derivation call sites resolve to a declared signature instead of an empty one. It covers all five spellings a consumer writes across the published range -- `Hkdf`/`HkdfExtract`, the `SimpleHkdf`/`SimpleHkdfExtract` aliases added in 0.12.1, and the `GenericHkdf`/`GenericHkdfExtract` types that became the real ones in 0.13.0 -- because the parser keys a call by the path the consumer wrote. `new` is declared at both of its arities: 0.1.0 selects the digest by name and takes three arguments, every release from 0.8.0 takes an optional salt and two. Returns follow the `return.type` / `canonical_return_type` split, so `extract` declares the crypto object it yields rather than the tuple that wraps it.
- A callgraph contract KB for the `md5` crate (pkg:cargo/md5), so a consumer's one-shot and incremental MD5 call sites resolve to a declared signature instead of an empty one. It covers the crate-root `compute` free function, the `Context` lifecycle including the `io::Write` input path, and `Digest::from`, and it declares `finalize` (0.8.0) beside the deprecated `compute` it replaced rather than treating either as an era replacement. This crate is NOT RustCrypto's `md-5`, and both compile to the lib name `md5`, so a consumer writes `md5::` for both and the crate name never appears in the source; the two KBs are separated by their key sets alone -- `md-5.yaml` keys on `md5::Md5`, this one on `md5::Context`, `md5::Digest` and `md5.compute` -- and a test asserts the two sets stay disjoint. It ships as two files because `version_range` is declared per library and may only claim versions for which every entry is true: `Context::finalize` and `Context::default` arrive at 0.8.0, so they live in `md5-0.8.yaml` with their own range rather than widening the main file's.
- A callgraph contract KB for the `secp256k1` crate (Rust bindings to libsecp256k1), so a consumer's ECDSA, BIP-340 Schnorr, ECDH and BIP-324 ElligatorSwift call sites resolve to a declared signature instead of an empty one. It carries both module spellings and both arities wherever the range disagrees: `secp256k1::key::SecretKey::new` is the only spelling that compiles on 0.1.0-0.20.x and a different key from the crate-root re-export, `SharedSecret::new` takes the context as its first argument up to 0.14, and `KeyPair` becomes `Keypair` at 0.28.0. Sixteen entries declare no `parameter_types`: the message argument changed twice inside the range -- `&Message` to `impl Into<Message>` at 0.31.0 for the ECDSA methods, and `&Message` to `&[u8]` at 0.30.0 for the Schnorr ones -- so method plus arity does not identify one signature there, and declaring one era's would be wrong for the other. Their roles and return types are still declared, which is what a chain walk needs.
- A callgraph contract KB for the `blake3` crate, so a consumer's BLAKE3 hashing, keyed-hashing and key-derivation call sites resolve to a declared signature instead of an empty one. All 42 entries declare `parameter_types` and `canonical_return_type`, which is what fills that signature -- without them a contracted call still renders as `name(?)`, because the export reads only those two fields. It covers the inherent API, the hasher lifecycle including the `io::Write` input path that was the only way to stream a file in before 1.5.0, the extendable-output reader through both `fill` and its `XofReader`/`Read`/`Seek` trait forms, the 1.8 `hazmat` tree primitives, the three `HasherExt` chunk methods, and the RustCrypto `Digest`/`Mac` surface -- `blake3::Hasher` is a full RustCrypto MAC, so `new_from_slice` and `verify_slice` are contracted on the keyed side and `new_with_prefix`, `digest` and the `finalize_*` family on the unkeyed one. `derive_key` is declared at both of its arities, because it wrote the derived bytes into a caller-supplied buffer through 0.3.8 and returns them from 1.0.0, and that buffer's length is declared as the derived key size since it is the only place that length is knowable. The `hazmat` entries keep their `blake3::hazmat` module segment while the crate-root `Hasher` and `OutputReader` carry none. Returns are read from the source rather than by symmetry: `merge_subtrees_root` yields a `blake3::Hash`, only the non-root form yields a `ChainingValue`, and the xof form an `OutputReader`.
- A callgraph contract KB for the `bls-signatures` crate (BLS signatures over BLS12-381, as used by Filecoin), so a consumer's signing, verification, aggregation and key call sites resolve to a declared signature instead of an empty one. All twenty entries declare `parameter_types` and, where method plus arity identifies one signature, `canonical_return_type`, which is what fills that signature. It fixes wrong attributions as well as absent ones: on a probe consumer, mis-attributed symbols fall from 7 to 3, and the four it closes were all typed on `std::Vec` — including the receiver of `sk.public_key().verify(sig, msg)`, so a served report named Vec's method for a BLS verification. That particular outcome needs a consumer that declares a same-named `public_key()` of its own returning `Vec<u8>`, which a wrapper layer around any signature scheme tends to have; without one the same call resolved to the consumer's own package instead. The whole `Serialize` trait is covered on all three types, `write_bytes` included. Three curve-point types are declared crate-unqualified on purpose: `pairing` and `blst` are mutually exclusive features and the point types come from whichever is compiled, so naming one crate would publish a dependency the scanned build may not have. The declared range is `>=0.7.0,<0.16.0` — 0.6.1 and below lack `verify_messages` or have an infallible `aggregate`, and 0.15.0 is the crate's last published version.
- A callgraph contract KB for the `biscuit` crate (JOSE — JWT, JWS and JWE for Rust), so a consumer's signing, verification, encryption and key-wrapping call sites resolve to a declared signature instead of an empty one. Every type is contracted under TWO spellings because `JWT` and `JWE` are type aliases for `jws::Compact` and `jwe::Compact` and the Rust parser keys a call by the path the consumer wrote: `JWT::new_decoded(..)` emits `biscuit.JWT.new_decoded` while `jws::Compact::new_decoded(..)` emits `biscuit::jws.Compact.new_decoded`, and both are in real use — the crate's README uses the aliases, its own tests use the full paths. It ships as four files rather than one because `version_range` is declared per library and may only claim versions for which every entry is true: the core API goes back to 0.0.8, `decode_with_jwks` arrives at 0.5.0 (`biscuit-jwks`), the `Signable`/`SignedData` surface at 0.6.0 (`biscuit-flattened`), and `decode_with_jwks_ignore_kid` -- which reads like a sibling of `decode_with_jwks` and is not -- only at 0.7.0 (`biscuit-jwks-ignore-kid`). `SignedData::verify_flattened` is declared at arity 3, because it is an associated function taking `(data, secret, algorithm)` and not a method on the signed value; `Signable::new` carries `serde_json::Error` rather than the crate's own error type, because it fails while serializing the protected header before any crypto happens. `unverified_payload` is deliberately uncontracted: it returns the caller's own claims type, so there is nothing to declare.
- A callgraph contract KB for the `bcrypt` crate, so a consumer's password hashing and verification call sites resolve to a declared signature instead of an empty one. The crate's callable surface is free functions in a single-segment module, so every key is the `bcrypt.<fn>` dot form. Eleven of the fourteen entries declare `parameter_types` and `canonical_return_type`; the three that do not are the three where method plus arity does not identify one signature across the range — `hash` and `verify` changed their password type at 0.2.2 and `hash_with_salt` its salt type at 0.13.0. Cost is declared as an argument rather than encoded: every hashing function takes it at the call site. The stored digest that `verify` checks against is `passwordHash`, the same property `golang-x-crypto`'s `CompareHashAndPassword` uses for the same argument, rather than `ciphertext`. The raw Eksblowfish primitive is declared at both of its arities, because it wrote into a caller-supplied buffer through 0.12.1 and returns the digest from 0.13.0, and recording only the modern arity would leave every pre-0.13 call site unresolved while looking complete.
- A callgraph contract KB for the `balloon` crate (Balloon memory-hard password hashing), so a consumer's hashing, verification and instance call sites resolve to a declared signature instead of an empty one. The two crate-root free functions emit a one-dot key (`balloon.balloon`) and the `Balloon<D>` struct emits two (`balloon.Balloon.new`), so both spellings are authored as the graph emits them. The declared range is `>=0.0.14` even though five of the six entries already hold at 0.0.13: `verify` returns `Result<bool, Error>` at 0.0.13 and only becomes `Result<(), Error>` at 0.0.14, and `version_range` is declared per library rather than per entry, so it cannot cover a version for which any one declared signature is false.
- A callgraph contract KB for the `aws-sdk-kms` crate (the generated AWS SDK for Rust client for AWS Key Management Service), so a consumer's KMS call sites resolve to a declared signature instead of an empty one. Without it the client method resolves and every later link of the builder chain comes back as the consumer's own package, which is what an absent contract looks like in an exported call graph. It covers both builder generations, because the type was renamed and moved at 0.39.0, and each generation's method set is extracted from that generation's sources rather than copied across: the pre-0.39.0 builders have no `dry_run` and no `recipient`, and no legacy `DeriveSharedSecret` exists, because that operation arrives at 1.32.0 after the legacy module was deleted.
- A callgraph contract KB for the `security-framework` crate (Apple Security.framework bindings), so a consumer's SecKey, SecTrust, SecPolicy, SecRandom, PKCS#12 and Secure Transport call sites resolve to a declared signature instead of an empty one. The crate re-exports nothing from its root, so every key keeps its defining module segment — `security_framework::key::SecKey.create_signature`, `security_framework::os::macos::encrypt_transform::Builder.mode`.
- A callgraph contract KB for the `ssh2` crate (Rust bindings to libssh2), so a consumer's SSH handshake, public-key and password authentication, host-key and known-hosts call sites resolve to a declared signature instead of an empty one. It spans the whole published range across the 0.4.0 cutover: `handshake` is declared at both of the arities its signature took, because it takes a raw socket in 0.1.0-0.1.7, a `&TcpStream` in 0.1.8-0.3.3 and nothing from 0.4.0 on, and recording only the modern arity left every pre-0.4.0 consumer unresolved for the one call the family is about. Where method plus arity does not identify one signature -- `handshake` at arity 1 spans two different parameter types -- `parameter_types` is omitted rather than declared as one era's and wrong for the other. `host_key` and `host_key_hash` carry `role: output` rather than `operation`, since both hand back material the session already holds.
- A callgraph contract KB for the `snow` crate (the Noise Protocol Framework in Rust), so a consumer's handshake, transport and key call sites resolve to a declared signature instead of an empty one. It covers both eras of the crate: the 0.3.0+ `Builder`/`HandshakeState`/`TransportState` surface and the 0.1.x/0.2.x `NoiseBuilder`/`Session` one, which is not an alias — the older key generator is `generate_private_key` returning `Result<Vec<u8>>`, and `generate_keypair` arrives only with the rename. `StatelessTransportState` read/write are declared at arity 3, because that variant takes an explicit nonce.
- A callgraph contract KB for the `schannel` crate (Rust bindings to Windows SChannel and CryptoAPI/CNG), so a consumer's TLS handshake, credential acquisition, certificate parsing, fingerprinting and private-key import call sites resolve to a declared signature instead of an empty one. The range starts at 0.1.8 rather than at the crate's first release: 0.0.1 and 0.0.2 are a different crate whose entire source is `lib.rs` plus `hyperimpl.rs`, and 0.1.0-0.1.7 have no `crypt_prov` module, so the key-container and private-key import entries would be unreachable. It also declares the builder config methods and the `ProviderType`/`HashAlgorithm` constructors with their return types: schannel's API is fluent builders end to end, and those are the links a chain walk has to travel to reach an operation.
- A callgraph contract KB for the `tfhe` crate (Zama's TFHE-rs fully homomorphic encryption), so a consumer's key generation, encryption and decryption call sites resolve to a declared signature instead of an empty one. It covers the crate's three consumer-facing layers, whose keys are spelled differently: the high-level API is re-exported from the crate root and emits `tfhe.FheUint32.encrypt`, while the integer layer is not and emits `tfhe::integer.RadixClientKey.encrypt_radix`. The range starts at 0.1.0 because tfhe 0.0.0 is a name placeholder whose entire source is `pub fn placeholder() {}`.
- A callgraph contract KB for the `thrussh` SSH crate, so a consumer's connection, authentication and key call sites resolve to a declared signature instead of an empty one. It spans the whole published range: `load_secret_key` and `generate_keypair` are declared at both of the arities their signatures took, the authentication receiver is declared as both `client::Connection` (up to 0.21.x) and `client::Handle` (0.22.0 onwards), and the pre-rename `authenticate_key` spelling is kept alongside `authenticate_publickey`.

- Callgraph export schema `6.15` is available as an opt-in interned render (`--export-callgraph-interned-frames` / `ScanMeta.InternedFrames`): `call_chains` frames omit catalog identity and join through `functions[]` plus `finding_graphs[].call_chain_indexes`. Hop-specific fields stay on the frames. **Default live `--export-callgraph` and stitch remain schema `6.14`** with identity still inlined on every frame, until Earnie and scanoss.api hydrate from the catalog. `HydrateChainIdentities` copies catalog identity onto empty frames so both renders join the same way. The published interned schema is `schemas/callgraph-schema-6.15.json`.
- `--export-callgraph-interned-frames` stamps schema `6.15`. Parsers that only read `call_chains[]` objects must not enable it until they gate on `schema_version` or hydrate from `functions[]`.
...
### Fixed
- A Rust `use` declaration written as a BARE brace group -- `use { crate::config::Version, blstrs::{Bls12, G1Affine} };` with no path in front of the brace -- now binds its items. The whole declaration was skipped before, so every type it brought in resolved to the SCANNED crate rather than the crate it came from: measured on agave-bls12-381 4.0.3, whose `src/pairing.rs` uses this form, the call graph emitted `agave_bls12_381::pairing.Bls12.multi_miller_loop(?)` where the one-import-per-line spelling of the same code emits `blstrs.Bls12.multi_miller_loop(..)` -- a wrong identity that no contract can join, and one that mistypes the receiver of any call chained onto it. Every item shape inside the group now binds exactly as it does written one import per line -- a plain crate name, a `crate::`-rooted path, a braced sub-group, a glob and an alias -- which is what the accompanying test compares, shape by shape. The form is what `rustfmt` produces under `imports_granularity=One`. It also closes the nested-brace gap that left an implemented trait unqualified, which `rust_impl_trait_test.go` had pinned as a known gap with a note to assert the fixed value once it landed.
- Rust findings now carry their supporting calls and their fluent-chain links. The Rust parser was the only one of the seven that emitted neither `ChainID` nor `AssignedVar`, and set `ReceiverVar` only on call shapes whose receiver had NOT resolved -- so on a resolved crypto call, the one that matters, the object had no identity at all. Every reader treats an empty value as "no chain here" rather than "unknown", so `resolveFluentChainCalleesByContract`, `deriveObjectLifecycleCalls` and the chain-root tie-break in terminal selection were not degraded for Rust, they were inert. Measured on `native-tls-gmssl 0.1.0`, a published schannel consumer: the same 8 cryptographic assets, and supporting calls 0 -> 19. Detection is unchanged -- this is the reachability layer, not the rules.
- The contract-driven fluent-chain rewrite now spells a Rust callee the way the Rust parser does. `resolveChainLinkCallees` split the propagated receiver type on the last `.` and appended Java's `#<arity>` suffix, which turned `schannel::tls_stream::Builder` into a callee with an empty package, the whole `::` path in the type field and an arity of -1 -- an identity that joins nothing. The pass was unreachable for Rust until `ChainID` was populated, so nothing had exercised it.
- A Rust associated function's return type now comes from the contract KB before the path it was called on. `rustScopedCallType` returned the path unconditionally, which is right for `Type::new()` and wrong for every builder accessor: `SchannelCred::builder()` typed as `SchannelCred`, so `b.acquire(..)` on the bound variable resolved against a type that does not have the method. The path stays the fallback, and a declared return that names a bare value wrapper with no generic argument (`core::result::Result` with nothing inside, which several RustCrypto contracts honestly declare) is rejected rather than taken, because it loses the type the path already gave. The four mirror entries `schannel.yaml` carried on `SchannelCred` -- methods that type does not own, declared only so the wrong identity would join -- are deleted with this change, as their own note instructed.
- A Rust method call chained onto another method call on a third-party type now resolves to the crate that owns it. Only the scanned source's own declarations were consulted for a receiver's type, so `b.padding(..).mode(..)` typed `padding` correctly and emitted `mode` under the SCANNED crate's package — a wrong identity that looks resolved and matches no contract. The contracts KB now supplies the return type for that hop, reading it from `return.type` (the resolution type) rather than `canonical_return_type` (the full declared one); an uncontracted method still resolves to nothing rather than borrowing the receiver's type, and a comment inside an argument list no longer inflates the arity past the contract lookup.
- A crypto operation the parser attributes to no containing function no longer costs a whole component its reachability. A match inside a `#[cfg(test)] mod`, a macro body or a static initializer has no declared enclosing function, and the stitch self-chain fallback used to fabricate a frame for it with NO identity at all: no function key, no name, no signature. The interned frame form cannot express that frame — identity lives only in the `functions` catalog, so the entry holds nothing to hydrate from — and a consumer that requires hydrated frames rejected the entire component over one unattributable op, dropping the reachability of every well-formed finding in it too. Such findings are now served with empty `call_chains` and `reachability: not_applicable`, which is what a live scan already emits for the same operation; the finding's purl, its dependency-prefixed `finding_id` and the supporting calls on the same node are all preserved. Measured on one crate whose test modules exercise the matched API: 12 of 47 operations are unattributable, and they used to cost the reachability of all 47.
- A Rust rule no longer claims a crate for a FREE FUNCTION the scanned source declares in its own module. opengrep's Rust engine does not distinguish a method call from a path call — measured on 1.12.1 and 1.28.0, `$X.write_message(...)` matches `t.write_message(..)`, `framing::write_message(..)` and `Codec::write_message(c, ..)` alike — so an import-guarded rule also claimed the consumer's own module-level functions, and no receiver constraint written at the rule layer can separate them. The receiver filter judged only calls with an owning type and skipped free functions, so those matches survived: a probe consumer whose own `handshake` and `framing` modules declare `build_initiator`, `generate_keypair`, `psk`, `local_private_key`, `write_message` and `rekey_outgoing` was served six false `pkg:cargo/snow` assets. A free function now takes its crate from the module it is declared in, judged by the same conditions a method gets — so it is dropped only when the scanned source itself declares it under a crate the rule does not name, and a genuine call whose receiver could not be typed is still kept.
- A Rust crate's rules no longer claim crypto performed by a type the scanned source declares itself. A Rust rule identifies its crate with a file-level import guard (`pattern-inside: use $IMPORT; ...` plus a `metavariable-regex`), which anchors the FILE and not the CALL: in any file that imports the crate, a rule matching `$KEY.sign($MSG)` also claimed a same-named method on the consumer's own type, and that attribution reached the served report as a fact about the component. The guard also matches a LOCAL MODULE of the same name, so a crate that writes `mod sha1;` and implements SHA-1 by hand was given sha1's PURL. Matches are now dropped when every call node at the match position resolves to a method the scanned source itself declares, under a crate the rule does not name — the receiver type is knowable from the call graph, which is why this belongs here rather than in the rules. Deliberately one-sided: an unresolved receiver, a chain that reaches the crate through a later link, a method of the crate's own trait implemented on a local type, and a method whose name carries the crate's name (`to_pkcs8_der`) are all kept. Rust only; the construct it compensates for exists in no other language. Verified over 206 published reverse-dependencies (~4,500 files): every drop is a wrong-PURL attribution, none is a lost detection.
- A Rust fluent chain no longer has a correctly-resolved dependency callee overwritten by a same-named local method. The chain pass re-qualifies each link from the previous link's return type through a normalization that reduces `ed25519_dalek::Keypair` to the bare `Keypair`, then looks that name up in an index built only from types the scanned source declares. A wrapper crate that names its own type after the one it wraps — `impl Keypair { fn to_keypair(self) -> ed25519_dalek::Keypair; fn sign(..) { self.to_keypair().sign(msg) } }`, published as tlfs-crdt 0.1.0 — had its `sign` link rewritten from the crate to itself, so the call joined the wrapper's contract instead of the crate's. The pass now carries the module path the return type was declared with and declines to re-qualify when it names a different unit. An unqualified return type keeps the previous behaviour, so Java fluent chains are untouched.
- Rust `impl <Trait> for <Type>` blocks record the trait they implement, with its path resolved through the file's imports. The impl header's trait field was read and discarded, so a crate's trait method implemented on a local type — apple-codesign 0.16.0's `impl EncodePrivateKey for InMemoryPrivateKey`, where `to_pkcs8_der` is pkcs8's API — was indistinguishable from a same-named inherent method. Recorded in a new `OwnerTraits` field rather than `OwnerBases`, which feeds Python subclass dispatch and the fragment export's type-hierarchy recovery; it stays nil for every other ecosystem and for inherent impls.
- Docker dependency-image builds (and therefore tagged GitHub Releases) no longer fail because the live rustup installer at `https://sh.rustup.rs` changed checksum. Images still install the pinned Rust 1.88.0 toolchain, now via a versioned `rustup-init` 1.29.1 binary from the rustup archive with checksum verification.
...
- Docker image builds (and therefore tagged GitHub Releases) no longer fail because the OpenGrep installer consults GitHub's first page of releases and no longer lists the pinned `v1.12.1`. Images still ship that same pinned binary, downloaded directly from the GitHub release with checksum verification. Scanner missing/too-old errors now point at that release instead of the installer.
- Four more differential corpora — grpc-go, etcd, caddy and cosign, 62,540 further resolvable call sites — hold the line: 100% of the calls go/types sees are seen (seven etcd sites excepted, an old-grammar misparse of user code that shadows `new` with a generic helper, documented alongside the generic-instantiation limitation), every dynamic site is accounted for, and zero calls resolve to a different member than the official answer. Four shapes were closed on the way: two `init` functions in one file kept colliding (grpc's service_config.go — the discriminator now carries a sequence number); a package's own `print` or `new` survives the predeclared pruning, which moved to the builder where the whole package is in view (etcd declares both); a call of a type-asserted func value (`hook.(func(string) *Result)(arg)`, grpc's internal hook wiring) is emitted honestly instead of vanishing; and a receiver typed by an anonymous composite (`x.(interface{ SetConfig(...) })`, caddy) carries the honest empty identity — its braces would not survive the identity round-trip anyway.
- The Go input set and three more call shapes now match the official toolchain, closing the differential on golang.org/x/crypto and tink-go to full site parity. A directory whose name begins with `_` is excluded, as the go tool excludes it — x/crypto's `_asm` avo generators alone contributed 7,850 call sites that no build contains. An explicitly instantiated generic call (`RegisterKeySerializer[*Key](&s{})`), which the pinned tree-sitter-go grammar misparses as a conversion to a generic type — silently, with no error node — is recovered as the call it is, restoring tink-go's 145 key-serializer registrations; a real conversion to a locally declared generic type is left alone. And an immediately invoked func literal (`go func(){...}()`) is emitted with the honest empty identity instead of vanishing. Differential state on the two libraries: 100% of the call sites go/types sees, every dynamic site accounted for (tink-go 268/268, x/crypto 79/79), zero destinations naming a different member, and x/crypto's only extra rows are the cross-platform files the host-targeted official build cannot see — deliberate for a scanner.
- A Go local shadows an imported package of the same name, as the language requires. `key.AESKeyBytes()` with a parameter `key *Key` next to `import ".../key"` resolved as a call into the package (tink-go's shape), and a name bound with no known type — `hash, ok := HashIdToHash(b)` beside `import "hash"` (x/crypto's shape) — did not shadow at all, because only typed bindings were recorded. Every `:=`- and range-bound name now registers as a local from its binding on; an unknown type stays empty, and the producer's return may still type it. Found by running the go/types differential over two further corpora (golang.org/x/crypto and tink-go): recall is 100% on both, and these were the only kind-level disagreements — zero calls resolve to a different member than the official answer across all four corpora measured.
- Go graph identities survive their collisions and match the official spelling where the corpus allows. Multiple `init` functions — Go permits any number per package — shared one graph key, so only the last one's calls survived; each now carries a file-discriminated identity, like the `<varinit:file>` declarations. Build-tagged platform variants of one function (`command_unix.go` / `command_windows.go`) union their calls instead of the later file silently dropping the earlier body, following the Rust collision-merge precedent. A receiver typed with its type arguments (`Reachable[T]`) erases them, so it joins the erased declarations. And a value-spelled call to a pointer-declared method is respelled to the declared form (`(T).m` → `(*T).m`), which is how go/types reports it — the same member now joins instead of dangling. Verified with a call-site-by-call-site differential against go/types over two corpora: 100% of the calls the official resolver sees are seen (age 1,576/1,576; this repository 9,148/9,148), zero calls resolve to a different member than the official answer, and the remaining spelling differences are confined to external types (method sets unknowable without signatures) and deliberate type-argument erasure.
- Measured against go/types itself — the official resolver, run over the same corpus and joined call site by call site — the Go parser now sees 100% of the calls the official tool sees (1,576/1,576 on filippo.io/age; it started at 94.3%), and 99.2% of resolved destinations match exactly, with every remaining difference being the same member under another spelling (a value receiver for an external pointer-receiver method, or the named interface where go/types reports the embedded declaring one). Three recall classes were closed on the way: a call whose receiver is a field chain (`s.c.XORKeyStream(...)`) was dropped from the graph entirely; a call in a package-level variable initializer (`var errNoMatch = errors.New(...)`) had no containing function and is now carried by a per-file synthetic `<varinit:file>` declaration — per file, because several files of one package collide on one map key and silently drop each other's calls; and binding collection is now in textual order, so `pk, ok := pk.(T)` resolves its own right-hand side against the outer binding, as Go's declared-before-use requires — a scope-level overlay silently typed the receiver against its own assertion.
- Go local bindings are scoped to the block that declares them (Go spec, "Declarations and scope"). One flat map per function let the type assertion in `if r, ok := r.(RecipientWithLabels); ok { ... }` rebind `r` for the whole function, so the outer `r`'s calls after the `if` were typed against the asserted interface — a silently wrong receiver, the same defect the Java parser had with sibling blocks. Bindings now overlay per scope; the assertion also binds its asserted type, in both the one- and two-valued forms.
- The remaining shapes that claimed a corpus identity with no declaration behind it are each handled for what they are: a struct's func-typed field (`DisplayMessage func(...)`) is declared as a member of its type, so calls through it join; an interface embedded in a same-file interface contributes its methods to the embedder; a conversion to a file-declared type (`WriterFunc(fn)`) is not recorded as a call, exactly like `int(x)`; a bare call through a local func value, a literal's parameter, or a func-typed method receiver carries the honest empty identity; and a receiver of predeclared type (`err error`) is never attributed to the caller's package. Measured on filippo.io/age: zero call sites now claim a corpus identity that no declaration backs, from 156 (9.9%) at the start of this series; the genuinely dynamic remainder is emitted unanchored.
- Go interface methods are declared, so calls through an interface-typed receiver now join and expand. An interface method has no body and the walk never produced a declaration for it: `id.Unwrap(...)` with `id age.Identity` had nothing to join, and the builder's generic interface dispatch — which keys off interface-owned declarations exactly as it does for Java — never fired for Go. Interface methods now carry `OwnerType` interface, the call joins them, and the dispatch expansion links the caller to every same-arity implementation, marked as `interface_dispatch` rather than presented as exact.
- A Go method call whose receiver type cannot be established is now emitted with no owner at all instead of under the caller's package. The old spelling — `cmd.StdoutPipe()` as `<caller package>.StdoutPipe` — looks like a package function, can coincide with a real same-package name, and, because an edge with no resolution entry reads as exact, presented an unresolved call as resolved proof. The empty form is the one the Java parser already emits, and the export layer's unresolved reporting picks it up. The assigned-var pass also runs to a fixed point, so a chained binding — `id, _ := age.GenerateX25519Identity(); r := id.Recipient(); r.Wrap(fileKey)` — types each receiver from the previous hop's contract, with pointer markers kept on the type side of the identity. Measured on filippo.io/age, calls claiming a corpus identity with no declaration behind it fall from 156 to 48 (9.9% to 3.1% of call sites).
- Go declared return types are now extracted at all. The declaration walk matched a child node *typed* `result`, but `result` is a field of the declaration node — the child's own type is `pointer_type`, `type_identifier`, or a `parameter_list` for multi-value returns — so the case never fired and every Go function carried an empty `ReturnType`. Reading the field (as the Java parser already does) fixes it, and feeds two consumers that were silently inert for Go: the fluent-chain return-type resolver, and the assigned-var pass below.
- A Go `:=` receiver bound from an in-corpus producer now types through that producer's declared return: `k := NewKeyring()` then `k.Unwrap(data)` joins `(*Keyring).Unwrap` instead of falling back to the caller's package. The first non-error component of a multi-value return is used, pointers and type arguments erased, and the pointer-receiver spelling is preferred when that is where the method is declared. A return spelled through the file's import alias (`*format.Header`) is expanded to its full package path at extraction, so a helper in one package returning a subpackage's type — age's own `Encrypt`/`encryptHdr` shape — types its receiver too. Measured on filippo.io/age, unresolved receivers fall further, 11.9% to 10.3% of call sites.
- A Go call chained on another call's result is now emitted and, when the chain roots at a KB-known producer, typed. `sha256.New().Sum(data)` produced only the `New` link: the selector walk recognized identifier receivers alone, so every chained call vanished from the graph — the crypto operation lost while its factory survived. The link now carries the honest empty-type callee plus the ChainID grouping, and the builder's existing contract pass resolves it (`hash.(Hash).Sum`). Two Go-shaped gaps in that pass were fixed on the way: Go callee names carry no arity suffix, so the contract key's arity now comes from the argument count (exact — the language has no overloading), and the Go KBs key methods on types in the `pkg.(Type).method` spelling, which the lookup never composed.
- A Go method call whose receiver was bound by `:=` from a KB-known producer now resolves through the producer's contract return type. `gcm, _ := cipher.NewGCM(block)` then `gcm.Seal(...)` fell back to the caller's package (`app.Seal`) — and could even join a same-package function that happened to share the method's name. It now resolves to `crypto/cipher.(AEAD).Seal`; the KB-typed receiver wins over the coincidental declaration, while a caller that also calls the plain function keeps that edge. Conservative on rebinding: a variable assigned two different contract types in one function is left alone — the per-function view cannot see block scope, and guessing there is how an AES receiver gets reported as a Mac. Measured on filippo.io/age: 31 previously invisible chained calls now exist, 18 previously unresolved receivers are typed, unresolved receivers fall from 13.3% to 11.9% of call sites.
- A Go method declared on a generic type now carries its receiver's identity. `func (g Gen[T]) Get()` parsed as a receiver shape the extractor did not recognize, so the method was registered with no owning type at all — an orphaned declaration no call could ever join; the pointer form kept the raw `[T]` in the identity, which a call on a `Gen[int]` equally cannot match. Both erase to the bare type name, as Go itself does when forming a method set. Found by running the parser over go/types' own corner-case suite.
- A Go `:=` declaration whose right-hand side states its type in its own syntax — a composite literal, a conversion, `make`, `new`, or `&` of a composite literal — now binds that type to the variable. Only `var x T` and parameters were read before, so a method call on any such local fell back to the caller's package. Binding the result of an ordinary call still needs the callee's return type and is unchanged.
...
### Added
- The new default-on `--export-callgraph-entry-points` flag can be set to `false` to omit the optional `crypto_entry_points` reverse-reachability index from `--export-callgraph`. When disabled, Crypto Finder skips constructing and writing the index while preserving `finding_graphs`, `functions`, and `supporting_calls`; existing exports are unchanged by default.
...
- Rust contracts KBs may declare `trait_associated_types`, so a generic bound to a known crypto trait (`KeyInit`, `Digest`, `BlockSizeUser`, `AeadCore`) resolves its associated type (`C::KeySize`) instead of stopping at the trait name.
- The Rust `elliptic-curve` crate's `Curve` trait is cataloged, so `C::FieldBytesSize` on a generic bound to `Curve` (as used by `p256`, `p384`, `k256`, `ecdsa`) resolves instead of stopping at the trait name.
- A Rust wrapper the analyzed crate declares `impl<T> Deref for Wrapper<T> { type Target = T; }` for is seen through to `T`, the same as the hardcoded `Box`/`Arc`/`Rc` table, without needing to catalog it — the shape crates like `secrecy` and `zeroize` use to hold key material.
### Fixed
...
- A Rust prelude type inferred from a constructor's return (`let v = Vec::from(p); v.clone();`) now resolves to `std`, as a written annotation already did, instead of the analyzed crate's own package.
- Scanning a Rust crate whose manifest name uses hyphens (`aes-gcm`) now attributes its own declarations to the underscore identifier (`aes_gcm`) a consumer's `use` actually resolves to, so a contract can match while scanning the crate itself.
...
### Added
- Rust callgraph contracts now cover the remaining `ed25519-dalek` signing and verification API spellings selected by Dalek detection: legacy `SecretKey.sign`, `Keypair.try_sign`, and the `sign_prehashed` and `verify_prehashed` operations on the applicable legacy and current key types.
...
### Added
- Rust callgraph contracts for the Azure Key Vault Rust SDK: the legacy all-in-one `azure_security_keyvault` client and the generated `_keys`, `_secrets` and `_certificates` crates. The two generations share method names and differ in arity — legacy `encrypt` takes two arguments and the generated one takes three — so a contract keyed on one shape silently fails to resolve the other's call sites, and both are contracted. These type the client surface only: Key Vault is remote and the crates run no cryptography, so nothing here asserts a local algorithm.
### Added
...
### Added
- Rust callgraph contracts for the Dalek curve suite: x25519-dalek key agreement, ed25519-dalek signing and verification, and curve25519-dalek base-point multiplication and scalar sampling. Each crate's range spans a public API rename, so both the old and the new spellings are contracted — `Keypair`/`PublicKey` alongside `SigningKey`/`VerifyingKey`, and the 0.1.0 free-function X25519 API alongside the key types that replaced it. Raw point arithmetic is deliberately uncontracted: it is group arithmetic rather than an API a consumer selects.
### Added
- Java callgraph contracts for AeroGear OTP 1.0.0, the one-time password library: Base32 secret generation, TOTP construction from a secret and from a secret with a clock, code generation and verification, the provisioning URI, the adjustable clock, and the HMAC primitive underneath. The hash argument of the HMAC constructor is operation-determining, so the algorithm is reported from the value the library passes at the call site rather than assumed, and the secret is exposed as a key-size parameter role. The library's own Base32 alphabet tables and its internal digit-truncation helpers are deliberately left uncontracted: they encode and format rather than being an API a consumer selects.
### Added
- C++ callgraph contracts for PicoSHA2, the header-only SHA-256 implementation: the one-shot hashing and hex-string entry points, the incremental hasher's construction, configuration, streaming update, finalization and byte output. The one-shot functions are contracted at every call-site arity a consumer can reach, since the iterator overload declares a defaulted buffer-size parameter and three further overloads share one arity. The hex-formatting helpers and the very-long-integer style internals are deliberately left uncontracted: they format or implement rather than being an API a consumer selects.
### Added
...
### Added
- Python key-derivation-function key-length resolution (`resolved_key_length`) now covers pyca/cryptography (`PBKDF2HMAC`, `Scrypt`, `HKDF`, `HKDFExpand`, `ConcatKDFHash`, `X963KDF`), the standard library (`hashlib.pbkdf2_hmac`, `hashlib.scrypt`), `argon2-cffi` (`PasswordHasher`, `low_level.hash_secret`), `bcrypt.kdf`, and `pycryptodome`/`pycryptodomex` (`PBKDF2`, `scrypt`, `HKDF`, `PBKDF1`), resolved from both positional and keyword-argument call forms. The field already exists in callgraph schema `6.13`; no schema change.
- Python dependency-mode type resolution: with dependency scanning enabled, a pip-resolved dependency's `.pyi` type stubs and annotated `.py` sources now contribute return types and class hierarchy to reachability, cached on disk per distribution so an unchanged dependency tree is not re-indexed.
- Python entries now carry `visibility`/`owner_visibility` following the naming convention: a dunder name is public, a single leading underscore is protected, a double leading underscore is private.
- Python receiver resolution now covers `@staticmethod`/`@classmethod`/`@property` semantics, `super()` resolved against the enclosing class's first declared base, bounded dynamic dispatch (`getattr(obj, "method")(...)`, `importlib.import_module(...)`, `__import__(...)` with a literal-string argument), `functools.partial` and `__call__` targets, and type-hint-driven resolution from parameter, return, and variable annotations (including `Optional[...]`/`Union[...]`/`X | None` normalization and string forward references).
...
### Fixed
- Python KDF key-length resolution now matches an exact keyword role before positional/type metadata, so reordered valid keywords such as `dklen=32, iterations=100000` report the requested output length rather than the value occupying the declared positional slot.
- Python dependency-mode resolution now keeps pip distribution coordinates separate from import namespaces (for example, `argon2-cffi` and `argon2`), excludes implicit method receivers from indexed parameter roles, closes each native parser worker, and invalidates cached signatures when the installed source bytes change.
- Python callgraph scope fidelity: parameters and locals now shadow same-named imports, decorator and default-expression calls stay in their enclosing execution scope, method signature metadata omits implicit `self`/`cls` receivers, and repeated assignments to one `self.attr` no longer merge terminal-operation lifecycles.
- Python parser throughput: each file is now walked once with per-scope deferred call resolution, bringing parse time back to the pre-parity baseline on large Python trees; a node-visit budget test guards against traversal regressions.
- Python callgraph reachability and supporting-call precision now matches Java's structural coverage:
  - A crypto object passed as a function/method parameter now resolves as a call receiver.
  - Non-assignment bindings (`with ... as`, `for ... in`, `except ... as`, the async variants, the walrus operator, tuple/star unpacking) now resolve `ReceiverVar`/`AssignedVar` like plain assignment.
  - `self.attr`/`cls.attr` assigned in any method of a class now resolves as a stable receiver identity across that class's methods.
  - Module-level and class-body statements that call crypto APIs outside any function now get a synthetic `<module>`/`<clinit>` entry point, scoped to that body's own direct statements.
  - Import resolution now recurses into `try`/`except`, `if` (including `TYPE_CHECKING` guards), and function bodies, resolves relative imports (`from . import x`, `from ..pkg import y`) against the current package, and binds a plain dotted import (`import a.b.c`) to its top-level name only.
  - A package's `__init__.py` re-exports (`from .mod import Name`) are propagated so a sibling module's `from pkg import Name` resolves to the declaring module; restricted to project-local packages so a dependency's public knowledge-base-keyed path is never rewritten.
  - An UPPER_CASE local variable now resolves as a call receiver instead of being mistaken for a type reference.
  - No export schema change; `internal/scan/supporting_calls.go` is unchanged.
### Added
