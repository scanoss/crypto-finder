...
## [Unreleased]
### Fixed
- Python callgraph reachability and supporting-call precision now matches Java's structural coverage. A crypto object passed as a function/method parameter now resolves as a call receiver, matching Java's parameter binding. Non-assignment bindings — `with ... as`, `for ... in`, `except ... as`, the async variants, the walrus operator (`:=`), and tuple/star unpacking — now resolve `ReceiverVar`/`AssignedVar` the same way plain `x = ...` assignment already did. `self.attr`/`cls.attr` assigned in any method of a class now resolves as a stable receiver identity shared across that class's methods, instead of being invisible to reachability. Module-level and class-body statements that call crypto APIs outside any function/method now get a synthetic `<module>`/`<clinit>` entry point, so those calls are reachable instead of silently dropped. Import resolution now recurses into `try`/`except`, `if` (e.g. `TYPE_CHECKING` guards), and function bodies instead of only the file's top-level statements, and correctly resolves relative imports (`from . import x`, `from ..pkg import y`) against the current package instead of mis-binding or dropping them. A package's `__init__.py` re-exports (`from .mod import Name`) are now propagated so a sibling module's `from pkg import Name` resolves to the symbol's real declaring module, gated on in-graph evidence so it never rewrites a public knowledge-base-keyed re-export path. No export schema change; `internal/scan/supporting_calls.go` is unchanged. (openspec/changes/python-parser-java-parity)
### Added
