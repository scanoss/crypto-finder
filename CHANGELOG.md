...
## [Unreleased]
### Added
- Callgraph export schema `6.15` is available as an opt-in interned render (`--export-callgraph-interned-frames` / `ScanMeta.InternedFrames`): `call_chains` frames omit catalog identity and join through `functions[]` plus `finding_graphs[].call_chain_indexes`. Hop-specific fields stay on the frames. **Default live `--export-callgraph` and stitch remain schema `6.14`** with identity still inlined on every frame, until Earnie and scanoss.api hydrate from the catalog. `HydrateChainIdentities` copies catalog identity onto empty frames so both renders join the same way.
