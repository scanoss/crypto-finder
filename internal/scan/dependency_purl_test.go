...
func TestDependencyContextFromEntity(t *testing.T) {
	...
	tests := []struct {
		...
		// node is a supported ecosystem: a scoped package keeps the @ in its
		// namespace, percent-encoded, which is the form the mining service's source
		// resolver reads back when it builds a registry tarball URL.
		{name: "npm-scoped", ecosystem: "node", module: "@scope/crypto", version: "1.0.0", want: "pkg:npm/%40scope/crypto@1.0.0"},
		{name: "unknown-ecosystem", ecosystem: "elixir", module: "crypto", version: "1.0.0"},
	}
