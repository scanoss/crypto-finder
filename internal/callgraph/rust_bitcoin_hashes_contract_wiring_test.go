...
func TestBitcoinHashesContractSetIsExact(t *testing.T) {
	...
	// `conf` is empty for the overwhelmingly common high-confidence entry and
	// spelled out only where the contract deliberately records something less.
	// It exists because the renderer used to hardcode `conf=high`, which made a
	// low-confidence entry inexpressible and would have pushed an author to
	// misdeclare the confidence rather than extend this table.
	type row struct {
		method string
		arity  int
		role   string
	...
	want := []row{
		{"bitcoin_hashes::Hash.hash", 1, "output", "bitcoin_hashes::Hash", []string{"&[u8]"}, "low"},
	...
	render := func(r row) string {
		...
		return fmt.Sprintf("%s#%d role=%s ret=%s params=[%s] conf=%s",
			r.method, r.arity, r.role, r.ret, strings.Join(r.params, ","), conf)
	}
