...
func wantPyjwtContracts() []string {
	return []string{
		...
		// decode_complete exists on the JWS layer exactly as on the JWT layer
		// (api_jws.py:219). Without this entry the node renders with NO return
		// type while its JWT-layer twin resolves to builtins.dict, so type
		// propagation stops at it.
		"jwt.api_jws.PyJWS.decode_complete#1 jwt.api_jws.PyJWS.decode_complete/operation/builtins.dict/high/-/builtins.dict/params=-/varargs=false/when=-/lib=pyjwt",
