...
func TestDependencyScanner_SourceScopeCache(t *testing.T) {
	...
	key := "library@1"
	nested := &entities.InterimReport{Findings: []entities.Finding{{FilePath: "node_modules/child/index.js", CryptographicAssets: []entities.CryptographicAsset{{}}}}}
	isolated := &entities.InterimReport{Findings: []entities.Finding{{FilePath: "index.js", CryptographicAssets: []entities.CryptographicAsset{{}}}}}
	cache := &fakeFindingsCache{getMap: map[string]*entities.InterimReport{key + ":hash": nested}}
	calls := 0
	...
	orchestrator := NewOrchestrator(&mockDetector{}, rules.NewManager(&mockRuleSource{}), reg)
	...
	if calls != 1 || goRes.report != isolated {
		t.Fatal("legacy key:hash entry must miss after hashed cache identity")
	}
	goKey := cache.putLastKey
	if !strings.HasPrefix(goKey, "dependency-findings-v2:") || goKey == key+":hash" {
		t.Fatalf("go put %q, want hashed identity", goKey)
	}
	cache.getMap[goKey] = nested

	nodeRes := scan("node")
	if nodeRes.err != nil {
		t.Fatal(nodeRes.err)
	}
	if calls != 2 || nodeRes.report != isolated {
		t.Fatal("npm reused another ecosystem's nested-source findings")
	}
	nodeKey := cache.putLastKey
	if nodeKey == goKey || nodeKey == key+":hash" || !strings.HasPrefix(nodeKey, "dependency-findings-v2:") {
		t.Fatalf("npm put %q, want a distinct hashed identity", nodeKey)
	}
	cache.getMap[nodeKey] = isolated

	if res := scan("go"); res.err != nil || calls != 2 || res.report != nested {
		t.Fatalf("go hashed identity not reusable: %+v, calls %d", res, calls)
	}
	if res := scan("node"); res.err != nil || calls != 2 || res.report != isolated {
		t.Fatalf("npm isolated cache not reusable: %+v, calls %d", res, calls)
	}
}
