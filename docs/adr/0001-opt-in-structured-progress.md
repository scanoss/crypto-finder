...
The v1 envelope is published as a JSON Schema and validated in tests. Additive optional fields remain within v1; breaking changes require a schema-version bump. Skip reasons are an open set within a schema version: a new reason may be added without a version bump, and a consumer must treat an unrecognized reason as "skipped, reason unrecognized" rather than as an invalid event, so a consumer validating against a vendored copy of the schema must not reject the event on the reason value alone. V1 reports lifecycle transitions, measured durations, dependency aggregate counts, and stable skip reasons only. It does not report percentages, estimates, per-file progress, or worker-level progress.
...
Optional `dependencies` and `export` phases emit `skipped` with `details.reason: "not_requested"` when their work is not requested. A requested dependency phase that cannot run emits `skipped` with one of `ecosystem_unknown`, `resolver_unavailable`, `parser_unavailable`, `callgraph_unavailable`, or `manifest_absent`, preserving the current warn-and-continue behavior; `manifest_absent` is the case where the ecosystem is known but neither the scan target nor any module directory discovered below it carries a manifest that ecosystem's resolver can read. A requested phase that resolves no dependencies completes with zero-valued counts. Child phases close before their parents, and concurrent dependency workers are represented only by aggregate dependency counts.
