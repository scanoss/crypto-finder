...
6. Enrichment + export    OID enrichment (internal/enricher); writers (internal/output,
   (internal/enricher,     internal/converter) emit interim JSON or CycloneDX CBOM;
    internal/scan,         --export-callgraph emits the schema-6.9 reachability export;
    pkg/graphfrag)         --export-graph-fragment emits a graph-fragment-1.10 fragment
```
...
| Package | Responsibility |
|---------|----------------|
| `graphfrag` | The graph-fragment model and wire schema (`graph-fragment-1.10`), fragment decode/encode, the tiered fail-closed **stitcher** that composes per-component fragments into transitive reachability, and the renderers (`ToCallgraphExport` — stamps callgraph schema `6.9` — and `ToFindingsEnvelope`). |
...
KB YAML schema version is `"2"` (internal to the loader). It is **independent** of the partner-facing export schemas (callgraph `6.9`, `graph-fragment-1.10`). See [AGENTS.md](../AGENTS.md#knowledge-base-layout-callgraph-inferred-types) for the authoring recipe.
...
| Callgraph export schema | `graphfrag.CallgraphSchemaVersion` | `6.9` | The partner-facing reachability contract changes |
| Graph-fragment schema | `graphfrag.SchemaVersion` | `graph-fragment-1.10` | The fragment wire format changes |
