...
Schema note: call graph export version `6.11` is current and carries optional direct finding `purl` values at the finding level, optional `occurrence_key` structural identities, canonical dependency package URLs inside `dependency_info.purl`, and contract-scoped `finding_graphs[].call_chains[].crypto_call.resolved_key_length` evidence for direct Java `KeyGenerator.init(int)` finding calls. Java runtime provenance remains available in `scan_metadata` for JDK-aware platform signature enrichment.
