...
- **`6.12`** adds the rule-vs-callgraph key-length conflict marker to `supporting_calls[].supporting_call.resolved_key_length`. When a detection rule declares a static `keyLength` and the callgraph resolves a different value for the same finding, the resolved `bits` stay primary, the rule value is retained as `rule_declared_bits`, and `rule_conflict` is `true`. Agreement, an unresolved key length, and a rule that declares no `keyLength` all leave both fields absent. The marker is computed during the scan, so consumers read it directly instead of re-deriving it from rule metadata.
...
- `supporting_calls[].supporting_call.resolved_key_length.rule_conflict` reports that the detection rule declared a static `keyLength` disagreeing with the resolved `bits`. The resolved value remains primary and the rule value is preserved in `rule_declared_bits`, so neither side is lost. Both fields are absent when the two agree, when no key length was resolved, or when the rule declared none. A supporting call reached by more than one finding carries the marker of the first finding that claimed it in the deduplicated `supporting_calls[]` array; join through `finding_graphs[].supporting_call_ids` when per-finding attribution matters.
